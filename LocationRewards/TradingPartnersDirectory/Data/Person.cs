...
namespace TradingPartnersDirectory.Data
{
	...
	public class Person
	{
		...
		[Property (Group = "Contact", Ldap = "imageUrl")]
		...
		public Uri GravatarUrl {
			get {
				return HasEmail ? TradingPartnersDirectory.Utilities.Gravatar.GetImageUrl (Email, 80) : null;
			}
		}
		...
		public string SafeDistanceRange {
			get {
				if (!string.IsNullOrWhiteSpace (LastName)) {

					decimal distanceInMiles = Decimal.Parse (LastName);
					string rangeDescriptor = "> 10";
					if (distanceInMiles >= 0 && distanceInMiles < 5)
						rangeDescriptor = "0 - 5";
					else if(distanceInMiles >= 5 && distanceInMiles < 10)
						rangeDescriptor = "5 - 10";
